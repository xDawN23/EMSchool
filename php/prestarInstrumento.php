<?php
    ...
    if ($estatus_instrumento != $aux) {
        $sql = "UPDATE instrumento SET estatus_instrumento = 'prestado' WHERE codigo_instrumento = $codigo_instrumento;";
        $guardar = mysqli_query($conexion, $sql);
        if ($guardar) {
            $_SESSION['mensaje'] = "<script>alert('Se ha prestado el instrumento con éxito.');</script>"; header("Location: prestamos.php");
        } else {
            $_SESSION['mensaje'] = "<script>alert('Ocurrió un error, intente de nuevo.');</script>"; header('Location: prestamos.php');
        }
    } else {
        $_SESSION['mensaje'] = "<script>alert('No se puede prestado el instrumento, porque el estatus es \"prestado\".');</script>"; header('Location: prestamos.php');
    }
